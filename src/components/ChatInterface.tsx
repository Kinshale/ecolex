...
When answering, always cite the specific law and article when applicable.`;
  ...
  const saveMessage = async (convId: string, role: 'user' | 'assistant', content: string, citations?: Citation[]) => {
    if (!user) return;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!conversationId) {
        conversationId = await createConversation(userMessageContent);
        if (!conversationId) {
          throw new Error('Failed to create conversation');
        }
        setCurrentConversationId(conversationId);
        navigate(`/law-chat?conversation=${conversationId}`, { replace: true });
      }
      ...
      await saveMessage(conversationId, 'assistant', data.content, data.citations);
    } catch (error) {
