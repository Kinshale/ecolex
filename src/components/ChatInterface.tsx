...
When answering, always cite the specific law and article when applicable.`;
  ...
  return <div className="flex flex-col h-full">
      ...
      {selectedLaws.length > 0 && <div className="border-b bg-muted/30 px-4 py-2 flex items-center gap-2 flex-shrink-0">
          <Scale className="w-4 h-4 text-primary" />
          <span className="text-sm font-medium">{selectedLaws.length} Laws Selected</span>
          <div className="flex-1 flex gap-1 overflow-x-auto">
            {selectedLaws.slice(0, 4).map(law => <Badge key={law.id} variant="secondary" className="text-xs flex-shrink-0">
                {law.short_name}
              </Badge>)}
            {selectedLaws.length > 4 && <Badge variant="outline" className="text-xs flex-shrink-0">
        ...
        {messages.length === 0 ? <div className="h-full flex items-center justify-center">
            <div className="text-center max-w-md animate-fade-in">
              ...
              <p className="text-muted-foreground mb-4">
                {selectedLaws.length > 0 ? `You have ${selectedLaws.length} laws selected. Ask questions about these regulations.` : 'Select laws and regulations to start your consultation.'}
              </p>
              {selectedLaws.length === 0 && <Button onClick={openModal}>
      ...
      <div className="border-t border-border p-4 bg-background">
        <form onSubmit={handleSubmit} className="max-w-3xl mx-auto">
          ...
          <p className="text-xs text-muted-foreground mt-2 text-center">
            {selectedLaws.length > 0 ? `Consulting ${selectedLaws.length} selected laws` : 'Select laws to get context-aware responses'}
          </p>
        </form>
      </div>
    </div>;
